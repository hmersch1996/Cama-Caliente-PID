...
float soak_setoint = 185;                       //Mode 1 soak is 150ºC for a few seconds
...
float MAX_PID_VALUE = 190;                      //Max PID value. You can change this. 
...
void loop() {
  //Primero se muestra en pantalla el valor medido por el termistor
  double temp = abs(therm1.analog2temp()); 
  if(temp>1000){temp=30;} //Si la placa aun no llega a una temperatura medible por el termistor, se muestra en pantalla que la placa esta a 30 grados Celsius
  else{temp=1.3171*temp + 60.728;} //Si la placa esta a una temperatura medible por el termistor, se realiza un ajuste de curva
  Serial.print((String)temp);
  //Serial.print(",");
  //Serial.println((String)tiempo);
  //delay(200); //wait 2000 mS for next measure <=estoy seguro de que esto era para la comunicacion serial
  //tiempo=tiempo+1;
  millis_now = millis();
  if(millis_now - millis_before_2 > pid_refresh_rate){    //Refresh rate of the PID
    millis_before_2 = millis(); 
    
    temperature = abs(therm1.analog2temp());
    if(temperature>1000){temperature=30;}
    else{temperature=1.3171*temperature + 60.728;}
    if(running_mode == 1){   
      //Primero preguntamos si la temperatura es menor a la temperatura de precalentado (~140-150C)
      if(temperature<preheat_setoint){temp_setpoint = seconds*1.666;}                    //Reach 150ºC till 90s (150/90=1.666)  
      //Luego preguntamos si ya alcanzo la temperatura de precalentado y si pasaron 90 segundos desde que inicio el modo 1  
      if(temperature>preheat_setoint && seconds < 90){temp_setpoint = soak_setoint;}   
      //Entre 90 y 110 segundos se realiza el proceso de soldadura superficial  
      else if(seconds > 90 && seconds < 110){temp_setpoint = reflow_setpoint;} 
      ...
      //De ser necesario, se corrige el valor maximo o minimo que puede llegar a tener el PID
      if(PID_Output>MAX_PID_VALUE){PID_Output=MAX_PID_VALUE;}
      else if (PID_Output < MIN_PID_VALUE){PID_Output = MIN_PID_VALUE;}

      //Enviamos una senhal PWM al rele
      ...
      PREV_ERROR = PID_ERROR; //se actualiza el valor del error anterior para el siguiente ciclo

      //Si superamos los 130 segundos, pasamos al siguiente modo
      if(seconds > 130){
  ...
  if(millis_now - millis_before > refresh_rate){          //Refresh rate of prntiong on the LCD
    ...
    //Mode 0 is with SSR OFF (we can select mode with buttons)
    if(running_mode == 0){ 
